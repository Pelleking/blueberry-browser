...
const topBarAPI = {
	...
	// Listen for offline mode changes from main
	onOfflineModeUpdated: (callback: (enabled: boolean) => void) => {
		electronAPI.ipcRenderer.on("offline-mode-updated", (_, enabled) => callback(enabled));
	},
	removeOfflineModeUpdatedListener: () => {
		electronAPI.ipcRenderer.removeAllListeners("offline-mode-updated");
	},

  // Bridge connection status
  onBridgeConnected: (callback: (connected: boolean) => void) => {
    electronAPI.ipcRenderer.on("bridge-connected", (_, connected) => callback(connected));
  },
  removeBridgeConnectedListener: () => {
    electronAPI.ipcRenderer.removeAllListeners("bridge-connected");
  },

	// Bridge QR
	generateBridgeQR: () => electronAPI.ipcRenderer.invoke("generate-bridge-qr"),
};
